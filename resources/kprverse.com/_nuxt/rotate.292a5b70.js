...
class x {
  constructor(
    x,
    {
      ...
      wrapS: m = d,
      wrapT: p = d,
      renderOptions: R = { target: null },
    } = {}
  ) {
    (this.renderer = x),
      (this.shader = c),
      (this.scene = new e()),
      (this.orthoCamera = new t(f / -2, f / 2, l / 2, l / -2, 1e-5, 1e3)),
    ...
    (this.fbo = {
      ...
      this.shader &&
        ((this.mesh = new s(new n(1, 1), this.shader)),
        this.mesh.scale.set(f, l, 1),
      ...
      e.removeFromParent(),
        null ==
          (n =
            null ==
            (s =
              null == (i = null == (t = e.material) ? void 0 : t.uniforms)
                ? void 0
                : i.tMap)
              ? void 0
              : s.value) || n.dispose(),
        null == (r = e.material) || r.dispose(),
        null == (a = e.geometry) || a.dispose(),
        (e = void 0);
    });
    for (const e in this.fbo)
      this.fbo[e] instanceof i &&
        (this.fbo[e].dispose(), (this.fbo[e] = void 0));
    this.fbo = void 0;
  }
  get texture() {
    return this.fbo.read.texture;
  }
  render({ final: e = !0, clear: t }) {
    e
...
const c =
  ...
  f =
    "#define GLSLIFY 1\nvec2 pos(in float x,in float y){return st0+vec2(x*rx,y*rx);}vec2 pos(in float x){return pos(x,x);}vec2 pos(in vec2 p){return pos(p.x,p.y);}float size(in float x){return x*rx;}vec2 size(in float x,in float y){return vec2(x*rx,y*rx);}",
  l =
